...
Project Manager 让你用自然语言管理项目，无需学习项目管理界面。只需谈论项目本身：目标是
什么、发生了什么、什么改变了、什么存在风险，以及必须做出什么决策。

## 你会得到什么

你会得到一份始终与现实保持一致的连贯项目计划。你提供有意义的信息：
...
Project Manager 判断这些信息会如何影响计划。一句话可能同时更新任务、依赖、排期、风险、
决策、证据和报告。文件与视图只是实现细节，不是用户必须掌握的语言。

你负责目标、约束、审批和决策。Project Manager 负责把它们转化为一致的项目状态。
...
Project Manager 会创建项目文件夹，定义可衡量的成功标准，并从目标反推包含必要任务和依赖
的可验证计划。如果不同理解会显著改变项目，它可能会要求你做出决策；它不应该要求你设计
字段或工作流。
...
项目通过事件、约束、证据和决策发生变化。出现这些事实时，直接告诉 Project Manager，
不必先把它们翻译成字段、状态或卡片移动。
